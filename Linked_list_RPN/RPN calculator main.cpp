...
//initializing box colors as global to
//avoid passing them through many functions
int boxColor[7][4] = {
    {13,13,13,13},
    {13,13,13,13},
    {15,15,15,15},
...
void command(){
    ...
    while(enter != "o"){
        ...
        //setting the coordinates of the list display title
        COORD coord;
        coord.X = 19;
        coord.Y = 6;
        SetConsoleCursorPosition(hWnd, coord);

        //setting the list display and the user input to the appropriate color
        ...
        //setting coordinates of the list display
        coord.X = 25;
        coord.Y = 6;
        SetConsoleCursorPosition(hWnd, coord);

        //displaying updated list
        for (int d = calc.Head(); d >= 0; d--){
            cout << calc.request(d) << " ";
        }
...
void number(apstring enter){
    ...
    //pushing entered number onto the start of the list
    calc.oprt(val);
}
